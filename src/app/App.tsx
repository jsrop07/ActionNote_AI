...
// - 입력 섹션 (텍스트 입력 / 파일 업로드)
// - 분석 결과 섹션 (요약, 통계, 결정사항, 액션 아이템, 리스크, 누락 정보)
// - 저장 섹션 (Google Docs 저장 / Markdown 다운로드)
// - 기존 Figma AI 생성 UI 디자인을 최대한 유지하면서
//   API 연결 상태 관리 틀만 추가함
// =====================================================================
...
export default function App() {
  ...
  const metrics = result
    ? calcMetrics(result)
    : { totalActionItems: 12, unassignedCount: 3, noDueDateCount: 5, riskCount: 2 };

  // ── 분석 핸들러 ──────────────────────────────────────────────────
  /**
   * 역할: "분석하기" 버튼 클릭 시 호출된다.
   * - 현재 활성 탭(텍스트/파일)에 따라 analyzeText 또는 analyzeFile을 호출
   * - TODO: 실제 API 연결 후 setResult(data)로 결과 반영
   */
  ...
  // ── Markdown 다운로드 핸들러 ──────────────────────────────────────
  /**
   * 역할: "Markdown 다운로드" 버튼 클릭 시 호출된다.
   * - TODO: 백엔드 API 또는 프론트엔드에서 직접 Markdown 생성 후 다운로드
   */
  function handleDownloadMarkdown() {
    ...
    // TODO: Markdown 텍스트 생성 후 Blob 다운로드 구현
    alert('TODO: Markdown 다운로드 기능을 구현해 주세요.');
  }
  ...
  const displayData = result ?? {
    summary: '2026년 6월 프로덕트 로드맵 회의에서 Q3 신규 기능 개발 우선순위를 논의했습니다. 모바일 앱 개선이 최우선으로 결정되었으며, 7월 말 베타 출시를 목표로 합니다.',
    decisions: [
      { content: '모바일 앱 개선을 Q3 최우선 과제로 결정', evidence_text: '모바일 사용자 비율이 60%를 넘었으므로 모바일 앱 개선을 최우선으로 진행하기로 결정' },
      { content: '7월 말 베타 버전 출시 목표', evidence_text: '7월 31일까지 베타 버전을 출시하여 사용자 피드백 수집' },
    ],
    action_items: [
      { task: '모바일 앱 UI 프로토타입 작성', owner: '김디자인', due_date: '2026-06-15', priority: '높음', evidence_text: '김디자인님이 다음주까지 프로토타입 완성' },
      { task: '백엔드 API 스펙 정의', owner: null, due_date: '2026-06-20', priority: '보통', evidence_text: 'API 스펙은 6월 20일까지 확정 필요' },
      { task: '테스트 환경 구축', owner: '이개발', due_date: null, priority: '낮음', evidence_text: '이개발님이 테스트 환경 준비' },
    ],
    risks: [
      { risk: '개발 리소스 부족 우려', response: null, evidence_text: '현재 개발팀 인원으로는 7월 말 일정이 타이트할 수 있음' },
      { risk: '외부 API 의존성', response: null, evidence_text: '결제 API 연동 지연 시 전체 일정에 영향' },
    ],
    missing_info: ['구체적인 예산 미확정', 'QA 계획 누락'],
  };
  ...
  return (
    <div className="min-h-screen bg-[#fafafa]">
      <div className="max-w-[1440px] mx-auto p-8">
          ...
          <div className="flex items-start justify-between gap-8">
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                요약, 결정사항, 액션 아이템, 리스크, 누락 정보를 구조화하고 Google Docs로 저장할 수 있습니다.
              </p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-border p-6">
            ...
            {activeInputTab === 'text' ? (
              ...
              <div className="border-2 border-dashed border-border rounded-lg p-12 text-center bg-muted/30">
                ...
                <p className="text-foreground mb-2">파일을 드래그하여 업로드</p>
                <p className="text-sm text-muted-foreground mb-4">
                  PDF, DOCX, XLSX, TXT 지원
                </p>
                {selectedFile && (
                  <p className="text-sm text-primary mb-2">선택됨: {selectedFile.name}</p>
                )}
                <button
        ...
        <div className="mb-8">
          {/* 요약 카드 — displayData.summary 표시 (API 연결 전: mock fallback) */}
          ...
          {/* 통계 카드 — metrics 변수에서 계산 (API 연결 전: mock fallback) */}
          <div className="grid grid-cols-4 gap-4 mb-8">
            <div className="bg-white rounded-xl shadow-sm border border-border p-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-muted-foreground">액션 아이템 수</span>
                <Target className="w-4 h-4 text-primary" />
              </div>
              <div className="text-3xl mb-1">{metrics.totalActionItems}</div>
              <p className="text-xs text-muted-foreground">총 액션 아이템</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-border p-6 mb-8">
          ...
          {/* 액션 아이템 탭 — displayData.action_items 동적 렌더링 */}
          {activeResultTab === 'actions' && (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-border p-6 mb-8">
          ...
          <div className="flex gap-3">
            ...
            {/* Markdown 다운로드 버튼 → handleDownloadMarkdown 연결 */}
            <button
